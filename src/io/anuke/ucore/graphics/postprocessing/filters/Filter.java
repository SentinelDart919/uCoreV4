...
public abstract class Filter<T> {
	 ...
	 * invoking endParams(). */
	// float
		protected T setParams (String param, float value) {
			try{
				if (!programBegan) {
					programBegan = true;
					program.begin();
				}
				program.setUniformf(param, value);
			}catch (Exception e){
				e.printStackTrace();
			}
			return (T)this;
		...
		// int version
		protected T setParams (String param, int value) {
			try{
				if (!programBegan) {
					programBegan = true;
					program.begin();
				}
				program.setUniformi(param, value);
			}catch (Exception e){
				e.printStackTrace();
			}
			return (T)this;
		...
		// vec2 version
		protected T setParams (String param, Vector2 value) {
			try{
				if (!programBegan) {
					programBegan = true;
					program.begin();
		...
		//vec3
		protected T setParams (String param, Vector3 value) {
			try{
				if (!programBegan) {
					programBegan = true;
					program.begin();
